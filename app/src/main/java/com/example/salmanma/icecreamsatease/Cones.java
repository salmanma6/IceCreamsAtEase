...
public class Cones extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setTitle("Cones");
        setSupportActionBar(toolbar);
        getSupportActionBar().setTitle("Cones");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        recyclerview=(RecyclerView)findViewById(R.id.listspice);
        recyclerview.setHasFixedSize(true);
        layoutManager = new LinearLayoutManager(getApplicationContext());
        recyclerview.setLayoutManager(layoutManager);
        reference=FirebaseDatabase.getInstance().getReference().child("products").child("type").child("cones");
            ...
            protected void populateViewHolder(final mnadptnew viewHolder, mcatnew model, int position) {
                try {
                    ...
                    Picasso.with(getApplicationContext())
                            .load(model.getLink())
                            .placeholder(R.drawable.defice).into(viewHolder.ppic);
